...
%% @copyright 2023-2025 Channel.me
%% @doc Web Push Api Subscription Check 
...
render_action(_TriggerId, _TargetId, Args, Context) ->
    ...
    OnSupported = proplists:get_all_values(on_supported, Args),
    ...
    OnSupportedScript = z_render:get_script(z_render:wire(lists:flatten(OnSupported), RenderContext)),

    Script = ["cotonic.broker.call('model/webPush/get/subscription') 
               .then( (msg) => {
                     const subscription = msg.payload;
                     if(!subscription.error) {", OnSupportedScript, "};
                     if(subscription.error) {", NotSupportedScript, "};
                     if(subscription.endpoint) {", SubscribedScript, "} else {", UnsubscribedScript, "}
               })
             "],
